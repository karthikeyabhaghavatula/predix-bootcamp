...
import com.ge.dspmicro.river.api.IRiverStatusCallback.ContentCategory;
import com.ge.dspmicro.river.api.TransferStatus.State;
import com.ge.dspmicro.httpriver.send.api.HttpRiverSendRequest;
        ...
        implements IRiverStatusCallback, Runnable
{
    ...
    //@formatter:off
    private static final String           HTTPRIVER_DIR  = ( (System.getProperty("predix.home.dir") == null) 
                                                         ? System.getProperty("user.dir") 
                                                         : System.getProperty("predix.home.dir"))
            ...
            throws Exception
    {
        ...
        HttpRiverSendRequest request = new HttpRiverSendRequest(srcFile);
        request.setContentType(fileDesc.contentType);
        request.setContentDescription(fileDesc.contentDescription);
        request.setContentDisposition(fileDesc.contentDisposition);

        // Send file
        UUID transferId;
        synchronized (_syncLock)
        {
            transferId = this.httpRiverSend.send(request);
        ...
        {
            _logger.info("SEND: success sending transfer with ID: " + transferId.toString());
        }
        else
        {
            _logger.error("SEND: failure sending transfer with ID: " + transferId.toString());
        }
        ...
        {
            _logger.info("RETRIEVE: success comparing retrieved file " + outFile.getName() + " with sent file "
                    + fileDesc.name);
        }
        else
        {
            _logger.error("RETRIEVE: failure comparing retrieved file " + outFile.getName() + " with sent file "
                    + fileDesc.name);
        }
    }
