...
IF %ERRORLEVEL% EQU 0 (
	(
	CALL :WRITECONSOLELOG "Predix Machine should not be run as root. This is poor practice for both system safety and system security.  If you must run with root privileges we recommend you create a low privileged predixuser, allowing them only the required root privileges to execute machine. Bypass this error message with the argument --force-root"
  	)
  	IF "%ROOT%" == "false" (
  		EXIT /B 1
  	)
...
IF EXIST "%PREDIXMACHINELOCK%\lock" (
	CALL :WRITECONSOLELOG "Lock file exists at %PREDIXMACHINELOCK%\lock.  This is created by Yeti at startup and removed by the stop_yeti.bat script.  Please shut down any other instances of Yeti running or delete this file manually."
	EXIT /B 1
)
