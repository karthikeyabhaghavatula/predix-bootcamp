...
predixuser, allowing them only the required root privileges to execute machine.  Bypass \
...
if [ -f "$PREDIXMACHINELOCK/lock" ]; then
	writeConsoleLog "Cannot obtain lock for Yeti process.  Close the other Yeti process running.  If no process is running delete the $PREDIXMACHINELOCK/lock file to override."
	exit 1
fi
...
while true; do
	...
	if [ "$zips" != "" ]; then
        for zip in "${PREDIXHOME}"/installations/*.zip; do
			# Verify the zip
			trap installInProgress exit INT TERM
